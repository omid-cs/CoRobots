...
class POMCP(object):
    ...
    def POMCP_search(self,samples,blackBox,threadID,timeout=None):
        if timeout == None:
        ...
        while (time.clock()-start) < timeout:
            ...
            self.POMCP_simulate(self.ucTree.root,thesamples[sampleindex],0,blackBox,threadID)
    ...
    def POMCP_simulate(self,ucNode,state,depth,blackBox,threadID):
        ...
        if (threadID=="agentThread" and state.turn=="agent") or (threadID=="clientThread" and state.turn=="client"):
            thereward = newreward+blackBox.discount_factor*self.POMCP_simulate(ucNode.children[besta][obsExists],newsample,depth+1,blackBox,threadID)
        else:
            thereward = blackBox.discount_factor*self.POMCP_simulate(ucNode.children[besta][obsExists],newsample,depth+1,blackBox,threadID)
